class Solution:
    ...
    def _get_area(self, events: list[tuple[int, int, int, int]], xs: list[int]) -> int:
        total_area = 0
        prev_y = 0
        tree = self._SegmentTree(xs)
        for y, delta, xl, xr in events:
    ...
    class _SegmentTree:
        ...
        def _add(self, tree_index: int, lo: int, hi: int, i: int, j: int, val: int) -> None:
